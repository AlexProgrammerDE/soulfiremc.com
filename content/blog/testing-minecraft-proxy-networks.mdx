...
<Callout type="info" title="What is a Proxy Network?">
...
**Single Server Setup:**
```text title="Single Server Architecture"
...
**Proxy Network Setup:**
```text title="Proxy Network Architecture"
...
[Velocity](https://papermc.io/software/velocity) is the modern, high-performance Minecraft proxy built by the PaperMC team.
...
<TabItem value="bungeecord" label="BungeeCord">

### BungeeCord

[BungeeCord](https://www.spigotmc.org/wiki/bungeecord/) is the original Minecraft proxy solution, widely used but showing its age.
...
<TabItem value="waterfall" label="Waterfall">

### Waterfall

[Waterfall](https://github.com/PaperMC/Waterfall) is a BungeeCord fork maintained by PaperMC with bug fixes and performance improvements.
...
```bash title="Download and Setup Velocity"
...
```bash title="Initial Velocity Startup"
...
```toml title="velocity.toml - Main Configuration"
# Server bind address
bind = "0.0.0.0:25577"
...
```bash title="Generate Random Secret"
# Linux/Mac
...
```properties title="server.properties - Backend Server Configuration"
...
```yaml title="spigot.yml - Backend Server Configuration"
settings:
  # IMPORTANT: Disable BungeeCord mode for Velocity
  bungeecord: false
```

**config/paper-global.yml** (all [Paper](https://papermc.io/) backend servers):

```yaml title="paper-global.yml - Velocity Forwarding Configuration"
proxies:
  velocity:
    enabled: true
...
```bash title="Starting All Servers"
# Terminal 1: Lobby
cd lobby-server
java -jar paper.jar
...
```bash title="Test Server Switching"
/server survival
...
```yaml title="config.yml - BungeeCord Configuration"
listeners:
  - host: 0.0.0.0:25577
    max_players: 500
    tab_size: 60
...
```yaml title="spigot.yml - BungeeCord Backend Configuration"
settings:
...
```properties title="server.properties - BungeeCord Backend"
online-mode=false  # Same as Velocity
```

## Player Info Forwarding Modes

<Callout type="info" title="Key Concept: Player Info Forwarding">
...
```toml title="velocity.toml - Modern Forwarding"
# velocity.toml
player-info-forwarding-mode = "modern"
forwarding-secret = "your-secret-here"
```

```yaml title="paper-global.yml - Modern Forwarding Backend"
...
```toml title="velocity.toml - Legacy Forwarding"
# velocity.toml
player-info-forwarding-mode = "legacy"
```

```yaml title="spigot.yml - Legacy Forwarding Backend"
# spigot.yml (backend)
settings:
  bungeecord: true
```
...
```bash title="BungeeGuard Installation"
...
```bash title="Authentication Test"
...
```bash title="Backend Direct Connection Test"
...
```python title="Theoretical Impersonation Test (Educational Only)"
...
```bash title="Server Switching Commands"
# Connect to proxy
...
```bash title="Connection Loss Test"
...
```toml title="velocity.toml - Fallback Server Configuration"
[servers]
lobby = "127.0.0.1:25565"
survival = "127.0.0.1:25566"
...
```text title="Concurrent Switching Test Scenario"
Test:
...
```bash title="Rapid Server Switching Test"
/server survival
/server creative
/server lobby
/server survival
...
```bash title="Connection Rate Benchmarks"
...
```bash title="Switching Speed Benchmarks"
...
```bash title="Monitor Proxy Resource Usage"
...
```text title="Gradual Load Test Phases"
...
```text title="Connection Burst Test"
...
```text title="Switching Storm Test"
...
```text title="Proxy Connection Test Scenario"
...
```text title="Server Switching Test Scenario"
...
```text title="Forwarding Validation Test Scenario"
...
```text title="Backend Failure Test Scenario"
...
```text title="Network Partition Test Scenario"
...
```text title="SoulFire Basic Proxy Test Setup"
...
```bash title="Test Proxy to Backend Connectivity"
...
1. Verify velocity.toml:
   ```toml title="velocity.toml - Check Forwarding Settings"
   player-info-forwarding-mode = "modern"
   forwarding-secret = "your-secret"
   ```

2. Verify paper-global.yml (backend):
   ```yaml title="paper-global.yml - Check Forwarding Settings"
   proxies:
     velocity:
       enabled: true
...
```bash title="Firewall Configuration"
...
```properties title="server.properties - Bind to Localhost"
# server.properties
server-ip=127.0.0.1
...
```toml title="velocity.toml - Increase Read Timeout"
# velocity.toml
[advanced]
# Increase timeout
...
<Callout type="success" title="Best Practices Checklist">
1. **Use [Velocity](https://papermc.io/software/velocity)** for new networks (better security, performance)
...
1. **[Velocity](https://papermc.io/software/velocity) Modern Forwarding** is the most secure option
