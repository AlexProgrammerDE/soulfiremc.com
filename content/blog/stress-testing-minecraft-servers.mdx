...
Without testing, you're gambling with your players' experience. A new plugin might work fine with 10 players and cause severe lag with 50. A VPS upgrade might not deliver the improvement you expected. Stress testing gives you hard numbers instead of guesses.
...
**G1GC (Aikar's flags)** divides the heap into regions and collects garbage in phases. It aims for a configurable pause time target (200ms in Aikar's config) but still produces stop-the-world pauses that scale with heap size. The extensive tuning parameters in Aikar's flags optimize G1's behavior for Minecraft's allocation patterns — lots of short-lived objects from chunk loading, entity processing, and packet handling. G1GC works well on modest hardware and smaller heap sizes.

**ZGC** takes a fundamentally different approach: nearly all garbage collection work happens concurrently with the application. Pause times stay under 1ms regardless of heap size, eliminating GC-related lag spikes — the kind that show up as random TPS drops or rubber-banding. ZGC is self-tuning and needs almost no configuration, but trades CPU cycles for those low pause times.

**Compact Object Headers** ([JEP 519](https://openjdk.org/jeps/519)) complement either collector. They shrink object headers from 12 bytes to 8 bytes, which reduces heap usage and improves cache locality. Minecraft creates millions of small, short-lived objects per tick, so the savings add up. On production workloads, this has shown up to 22% less heap usage and measurably faster execution.
