...
Stress testing simulates high player loads on your Minecraft server to find performance bottlenecks before real players hit them. The goal is not to see when the server crashes -- it is to understand how performance degrades under load and what capacity you can reliably support.

Without testing, you are gambling with your players' experience. A new plugin might work fine with 10 players and cause severe lag with 50. A VPS upgrade might not deliver the improvement you expected. Stress testing gives you hard numbers instead of guesses.

### Key Performance Metrics

Minecraft servers have two critical performance indicators: TPS and MSPT.

**TPS (Ticks Per Second)** measures the server's tick rate, targeting 20 ticks per second. **MSPT (Milliseconds Per Tick)** measures how long each tick takes to process. MSPT is often more revealing because it shows performance trends before TPS actually drops -- a server at 45ms MSPT is still hitting 20 TPS but has almost no headroom left.
...
Testing an unoptimized server wastes time. You will just discover it needs optimization. Establish a solid performance baseline first, then stress test to validate your capacity.
</Callout>
...
These are the most impactful settings you will adjust.
...
Use [Aikar's flags](https://aikar.co/2018/07/02/tuning-the-jvm-g1gc-garbage-collector-flags-for-minecraft/) for garbage collection tuning. Replace the memory values with your available RAM (Xms and Xmx should match):
...
With your server optimized and Spark installed, you are ready to test. The key is testing incrementally -- connecting 500 bots at once teaches you nothing except "it crashes."
...
Idle bots standing still do not represent real players. Add movement and actions to stress the systems that matter:

<Tabs>
<Tab value="movement" label="Movement" default>

Tests entity tracking, pathfinding, collision detection, and chunk loading. Even simple random walking provides valuable data. Monitor EntityAI CPU usage and chunk loading frequency in Spark.
...
Some issues only appear after hours: memory leaks, gradual resource exhaustion, increasing GC pause times. Connect 50-75% of your target capacity and let it run for 4-8 hours. Steadily climbing memory usage or gradually increasing MSPT signals a problem.

## Identifying Bottlenecks

After testing, your Spark profiles and recorded metrics should point to specific bottlenecks.
...
**Good TPS but high CPU (>80%)**: You are close to the limit with no headroom for spikes. Optimize further or upgrade hardware.
...
SoulFire is a Fabric-based bot tool that runs actual Minecraft client code. This matters for stress testing because traditional bot frameworks reimplement the protocol and physics, which can produce different packet timing, inaccurate physics, and detectable patterns that real players would not exhibit.

Because SoulFire bots are indistinguishable from real clients at the protocol level, the server load they generate accurately reflects what real players would cause. Combined with multi-version support via ViaFabricPlus, you can test against any Minecraft version.

**Basic test procedure:**

1. Set your target server and bot count in the SoulFire interface
2. Configure join delays (1-3 seconds between bots) for realistic connection patterns
3. Enable movement and activity plugins for realistic behavior
4. Monitor server performance with Spark during the test
5. Run for 15-30 minutes per load level

## When to Test

Stress testing is not a one-time activity. Run tests before launches, after major plugin updates, before events with expected high turnout, and after hardware changes. A monthly baseline test catches gradual degradation from accumulating data and slow memory leaks that daily monitoring misses.

The methodology is simple: optimize first, monitor with Spark, test incrementally with realistic behavior, fix what the profiles reveal, and retest. Do this before your players do it for you.
