...
**Layer 3/4 (Network/Transport):** This is where volumetric floods get absorbed, including SYN floods, UDP reflection, and amplification attacks. They're dropped at the edge before they consume bandwidth. This is the main reason you're paying for protection.
...
If you run Geyser for Bedrock players, you need UDP forwarding, and not every provider supports it. Bedrock players will silently fail to connect if it's missing. NeoProtect handles this well; confirm support with any other service before you commit.
...
For most servers, a subscription model is easier to budget for. Pay-as-you-go can spike hard if you're being actively targeted. That's especially worth remembering with CosmicGuard, which gets pricey at scale unless you're already on hosting that bundles it (BerryByte and PufferFish Host both use CosmicGuard infrastructure).
...
**Papyrus VIP** is worth considering if you need lots of geographic coverage. Seven standard locations is decent, and the Enterprise plan opens up 330+ locations. No one else really comes close there. They also offer protection for your website, voice chat, and TeamSpeak, not just Minecraft.

**CosmicGuard** has excellent filtering, but the pay-as-you-go model gets expensive at scale. It's a great deal if you're already hosting with BerryByte or PufferFish Host since they run on CosmicGuard's infrastructure, so the protection is already built in. Otherwise, budget carefully.
...
**Arvoris** is a newer entry from the team behind Papyrus. $9.99/month gets you unlimited usage across 7 locations. The pricing is simple, and there are no surprises. It is still building a track record, but the people behind it know what they're doing.
...
If you previously shared your origin IP in server lists, forum posts, or Discord, update it everywhere. Cached DNS entries and old links are the most common way backend IPs leak.
...
Your service will publish its IP ranges. Use those. This ensures that even if someone discovers your origin IP, direct connections get dropped.
...
After setup, join your server and check that your actual IP shows up in the logs instead of the proxy's IP. If every player shows the same IP, PROXY protocol isn't configured correctly and you need to fix it before going live.
...
For application-layer defense on top of DDoS protection, including anti-bot plugins, rate limiting, and behavioral analysis, see our [anti-bot testing guide](/blog/anti-bot-systems-testing-guide).
