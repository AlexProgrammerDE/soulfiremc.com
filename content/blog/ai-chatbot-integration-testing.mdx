...
**Trade-offs:** Requires an API key and internet connection. Costs roughly $0.01/1K input tokens for GPT-4. You are sending player chat data to OpenAI's servers, which may matter depending on your server's privacy stance.
...
Most AI chat plugins follow the same pattern: define a trigger keyword, set up a system prompt, configure rate limiting, and point at your LLM provider. Here is a typical configuration:
...
Getting AI chat running is straightforward. Making sure it actually works well under real conditions is where most setups fall short. Here are the areas that matter.
...
<Callout type="info">
Without rate limiting, a single player can burn through your API budget in minutes. This is not optional for cloud-hosted LLMs.
</Callout>
...
Manual testing covers functionality, but it cannot tell you what happens when 50 players ask questions at the same time during a server event. This is where bot-driven testing becomes essential.

**What to measure:**

Connect 10-50 bots and have them send AI queries simultaneously. Track:
...
- **AI service resource usage**: Monitor CPU and RAM on whatever is running your LLM. If Ollama hits 85%+ CPU during load tests, you will need to scale or queue more aggressively before a real event.
...
Every time you update a system prompt, swap models, or upgrade the plugin, re-run your test suite. AI behavior is inherently variable, so the goal is not identical responses but consistent quality. Keep a list of 20-30 representative questions and spot-check the answers after each change.
...
- **Prompt iteration**: Your first system prompt will not be your best. Review logs regularly and refine based on what players actually ask versus what the AI actually answers.

Running AI chat in production is an ongoing process, not a one-time setup. The servers that do it well treat their system prompts like code: version-controlled, tested, and continuously improved.
