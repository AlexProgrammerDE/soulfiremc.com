...
<Steps>
  <Step>
    ...
    **Hardware requirements by server size:**
    ...
    Red flags include HDD storage, less than 2GB RAM per 10 expected players, and shared hosting with unknown neighbors.

    Your backup system needs to actually work *before* your first crash, not after. Create a full backup, extract it to a separate directory, and confirm the world loads cleanly. Then schedule automated backups via cron: every 6 hours locally and daily offsite.
  ...
  <Step>
    ...
    **Configuration checklist:**
    ...
    - **World pregeneration:** Non-negotiable. Without it, 100 players exploring on launch day forces on-demand chunk generation and TPS drops to single digits. Use [Chunky](https://github.com/pop4959/Chunky) to pregenerate at least a 5,000-block radius from spawn. This can take hours, so plan accordingly.
  ...
  <Step>
    ...
    Common sources of trouble include multiple economy plugins conflicting, duplicate feature plugins (two claim systems), and version mismatches. For each critical plugin, including economy, permissions, claims, chat, teleportation, and anti-cheat, confirm it loads cleanly, test its primary features, test its interactions with other plugins, and check that permissions are correctly assigned.

    Walk through each rank (default, member, VIP, moderator, admin) and make sure players can use exactly the commands they should, and nothing more. Check chat prefixes, tab list formatting, and look for permission leaks that give default players access to moderation commands. This is the kind of thing that ruins trust fast.
  </Step>
  ...
  <Step>
    ...
    - **Mass join:** Simulate 50-100 players connecting within a few minutes. This is the launch surge. Keep an eye on connection failures, spawn area performance, and TPS dips during the initial flood.
  ...
  <Step>
    ...
    Your staff need to know what to do when things go wrong, because things will go wrong. Write up a quick-reference doc with the three or four most common issues: "can't claim land" (check their balance), "can't use command" (check their rank), "grief reported" (use `/co inspect`, rollback if needed). Make sure everyone has tested these commands on the actual server, not just read about them. Set up an on-call schedule so you have coverage during peak hours.
  ...
  <Step>
    ...
    Do a complete walkthrough of the player journey: join, orientation, resource gathering, claiming, building, social features, and economy. If something breaks during this walkthrough, fix it now, not tomorrow with 100 players online.
...
When you flip the switch, whether that means removing the whitelist or starting to add players, send your announcement and start monitoring. Watch player count, TPS, chat, and console errors. Staff should be actively welcoming players and answering questions, not lurking in a staff channel.
