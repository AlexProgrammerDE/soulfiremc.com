...
CLIs are for power-users that either like to only use their keyboard or want to automate bot sessions.
...
Now you need to decide, are we running the session locally on your machine, or are we using a remote dedicated server for the session?
...
```bash
java -Xmx$RAM -XX:+EnableDynamicAgentLoading -XX:+UnlockExperimentalVMOptions -XX:+UseZGC -XX:+UseCompactObjectHeaders -XX:+AlwaysActAsServerClassMachine -XX:+UseNUMA -XX:+UseFastUnorderedTimeStamps -XX:+UseVectorCmov -XX:+UseCriticalJavaThreadPriority -Dsf.flags.v2=true -jar SoulFireClient.jar --bot-address 127.0.0.1:25565 --bot-amount 100 --bot-protocol-version 1.20 --bot-join-delay-min 1000 --bot-join-delay-max 3000
```
