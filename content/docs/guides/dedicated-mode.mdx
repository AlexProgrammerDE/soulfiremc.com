...
and also a CLI can only use settings for sessions that were specified when the CLI was started up.
...
It allows anyone with access to it to connect to your server via the GUI or CLI and start bot sessions.
...
You've successfully set up a dedicated server. Now go to the GUI or CLI and connect to it and try running some bot sessions!
Dedicated servers use the same underlying code as integrated servers, so the experience should be the same.
Servers do most of the heavy work, they run the bot sessions, check proxies, authenticate accounts, and more.
