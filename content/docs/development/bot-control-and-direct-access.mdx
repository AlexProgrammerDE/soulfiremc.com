...
This is the part of plugin development that makes SoulFire powerful and risky at the same time.
Your plugin runs inside the same JVM as SoulFire and the headless Minecraft client, so you can work with the real runtime objects instead of sending remote commands to an external process.
