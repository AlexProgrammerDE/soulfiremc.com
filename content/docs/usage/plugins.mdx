---
title: Plugins
description: Plugins are a way to extend the functionality of SoulFire. SoulFire also comes with a few built-in plugins that can be enabled or disabled.
---
...
To install a plugin, you need to place the plugin `.jar` file in the `minecraft/mods` directory of the SoulFire server.
The SoulFire server will automatically load all `.jar` files in the `minecraft/mods` directory at startup.
...
you'll find the `minecraft/mods` directory in the `Help -> Data directory` folder in the `soulfire/minecraft/mods` subdirectory.
If you use a dedicated server,
you'll find the `minecraft/mods` directory located in the same directory as the server `.jar` file.
