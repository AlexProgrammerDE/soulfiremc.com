...
```bash
java -Xmx<ram you want> -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M -jar SoulFireClient.jar
```

Make sure to replace `<ram you want>` with the amount of ram you want to allocate to the server.
If you omit the entire `-Xmx<ram you want>` part, the CLI will use 25% of your total ram.
If your machine has 128GB of RAM and you want SF to use 120GB, you can use `-Xmx120G`.
If you connect to a remove server.
The ram can be extremely low, the RAM where the server runs matters.
...
|----------------------|------------------------|
| `"sf.remoteAddress"` | The ip to connect to   |
| `"sf.remoteToken"`   | The port to connect to |

You can add these as JVM properties before the `-jar` like so:

```bash
java -Dsf.remoteAddress=domain.com -Dsf.remoteToken=abcdefg -Xmx<ram you want> -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M -jar SoulFireClient.jar
```
...
You get a shell just like with a Minecraft server that allows you to run commands. Those are the same commands you get in GUI Mode.
