...
Both GUI and CLI come with a "integrated" server, which means the server is started and stopped when the GUI or CLI is started and stopped.
...
```bash
java -Xmx<ram you want> -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M -jar SoulFireDedicated.jar
```

Make sure to replace `<ram you want>` with the amount of ram you want to allocate to the server.
If you omit the entire `-Xmx<ram you want>` part, the server will use 25% of your total ram.
...
```bash
java -Dsf.grpc.port=12345 -Xmx<ram you want> -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M -jar SoulFireDedicated.jar
```
...
You can use the `generate-token` command to generate an access token.
